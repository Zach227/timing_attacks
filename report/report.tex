...
This bus was set up in previous labs to act as a one way communication channel for the MCU to send data to the FPGA.
...
When the FPGA is listening for a message from the MCU, it assigns the CM bus as high impedance so there are no conflicts from both entities driving the bus simultaneously.
...
Each delay is compared to a \verb|max_delay| regeister, and the \verb|max_delay| is updated as larger delays are recorded.
...
We were able to successfully perform this timing attack between the FPGA and the MCU.
The FPGA clock is fast enough for it to detect the added time to compare one more btye after the correct byte.
We successfully guessed keys ranging from 1 byte in length up to 32 bytes.
...
\begin{tabular}{cc}
\toprule
\textbf{Key Length (bytes)} & \textbf{Time (s)} \\
\midrule
1  & 0.3  \\
2  & 0.6  \\
4  & 0.9  \\
...
In comparison, we will estimate how long it would take to guess these keys with a brute force method.
...
These estimations are displayed in Table \ref{tab:byte_timing_bruteforce}.
...
\begin{tabular}{cc}
\toprule
\textbf{Key Length (bytes)} & \textbf{Time} \\
\midrule
1  & 0.225 Seconds \\
2  & $900\,\mu\text{s} \times 250^{2} = 56.25 \text{ Seconds}$  \\
4  & $900\,\mu\text{s} \times 250^{4} = 40.7 \text{ Days}$ \\
8  & $900\,\mu\text{s} \times 250^{8} = 4.41 \times 10^{8} \text{ Years}$  \\
\bottomrule
\end{tabular}
\label{tab:byte_timing_bruteforce}
\end{table}

These results demonstrate that our timing attack makes it possible to guess secret keys that would not be possible with a brute force approach.
